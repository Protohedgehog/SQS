...
# draw all occurrences and count genera seen at the quorum level(s)
...
for my $file ( @FILES )	{
	...
	for my $i ( 0..$#f )	{
		if ( $f[$i] =~ /occurrence(|s).genus_name/ )	{
			$fgen = $i;
		} elsif ( $f[$i] =~ /occurrence(|s).species_reso/ )	{
			$fspreso = $i;
		} elsif ( $f[$i] =~ /occurrence(|s).species_name/ )	{
			$fsp = $i;
		} elsif ( $f[$i] =~ /occurrence(|s).abund_value/ )	{
			$fabund = $i;
			...
			$fbin = $i;
		} elsif ( $f[$i] eq "10_my_bin" && $SCALE !~ /stages|epochs|fr2|peters/i && $SCALE =~ /[^0-9]/ )	{
			$fbin = $i;
...
if ( $METHOD =~ /CR|UW/ )	{
	open OUT,">./quorumSubsampleCurves4.txt";
	print OUT "items";
...
open OUT,">./quorumSubsampleRanges4.txt";
print OUT "genus";
...
open OUT,">./quorumSubsampleOccurrences4.txt";
print OUT "genus";
for $b ( $lastbin..$firstbin )	{
	print OUT "\t$binlist[$b]";
}
